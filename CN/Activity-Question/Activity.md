...
mkdir -p dir1/dir4 dir1/dir2/dir3 && cd dir1/dir2/dir3 # Creates nested directories using -p and changes into dir3
...
ls / ls -v # Lists directory contents; -v natural sort of version numbers
...
cp -n text newfile # Does not overwrite 'newfile' if it exists (-n = no-clobber)
cp -i text newfile # Prompts before overwriting 'newfile' (-i = interactive)
cp -u text newfile # Copies only if source is newer than destination (-u = update)
...
mv dir5/* dir1/dir6 # Moves all contents from dir5 to dir6 inside dir1
rm -v [a,e,i,o,u,A,E,I,O,U]* # Removes files starting with a vowel, verbose output
rm -v ???* # Removes files with at least 3 characters in name, verbose output
...
chmod 666 testfile3 # Sets read/write permissions for all using numeric mode
chmod 754 testfile3 # Sets rwx for user, r for group, r for others
chmod --reference testfile3 testfile2 # Applies permissions of testfile3 to testfile2
...
sudo chown -R user1:user1 dir2 # Changes ownership of dir2 and contents to user1
...
top  N P # Use 'top' command, then press N to sort by PID, P for CPU%
ps # Lists current terminal's processes
ps -e # Lists all system processes
df # Displays disk space usage of all partitions
tar -czvf mydir1.tar.gz dir1 # Creates a gzipped tarball of dir1
tar -xzvf mydir1.tar.gz -C dir5 # Extracts tar.gz file into dir5
```